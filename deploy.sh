...
# Set up Python virtual environment in current directory
...
# Get the current working directory (should be the git repo root)
CURRENT_DIR=$(pwd)
sudo tee /etc/supervisor/conf.d/rfid-attendance.conf > /dev/null <<EOF
[program:rfid-attendance]
directory=$CURRENT_DIR
command=$CURRENT_DIR/venv/bin/uvicorn app.main:app --host 0.0.0.0 --port 8000
user=ubuntu
autostart=true
autorestart=true
stdout_logfile=$CURRENT_DIR/logs/out.log
stderr_logfile=$CURRENT_DIR/logs/error.log
EOF
